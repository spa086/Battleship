...
public class WebAttackTests
{
    ...
    public void AttackMissed()
    {
        var game = SetupGameInPoolWithState(GameState.Player2Turn, 0,
            game => game.SetupSimpleFleets(new[] { new Cell(1, 1) }, new[] { new Cell(3, 3) }));
    ...
    public void AttackHitsAShip()
    {
        var game = SetupGameInPoolWithState(GameState.Player1Turn, 0,
            game => game.SetupSimpleFleets(new[] { new Cell(1, 1) }, new[] {new Cell(2, 2), new Cell(3, 3) }));
        ...
        Assert.That(decks.Where(x => x.Location == new Cell(2,2)).AssertSingle().Destroyed, Is.False);
    ...
    {
        var game = SetupGameInPoolWithState(GameState.Player1Turn, 0,
            game => game.SetupSimpleFleets(new[] { new Cell(0, 0) }, new[] { new Cell(0, 2)}));
